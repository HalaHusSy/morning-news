...
- **ภาพสินค้า**: โดยปริยายวาดเป็น SVG ในตัว (กล่อง retail + ตัวสินค้า + โลโก้แบรนด์ทางการจาก
  [simple-icons](https://simpleicons.org)) — อยากใช้**รูปถ่ายจริง** วางไฟล์ใน
  [`assets/hw/`](assets/hw/README.md) ตั้งชื่อ `<key>.jpg` แล้วรัน `npm run hardware`
  (หรือใส่ URL ในฟิลด์ `img`) ถ้ารูปโหลดพังเว็บจะ fallback เป็น SVG เอง
